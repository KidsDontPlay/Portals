...
public class Teleporter // todo give this class a full test when the dislocator
						// is updated to make sure everything still works
						// properly{
{
	...
	private static Entity teleportEntity(Entity entity, TeleportLocation destination) {
		...
		World destinationWorld = startWorld.getMinecraftServer().worldServerForDimension(destination.dimension);

		if (destinationWorld == null) {
			LimeLib.log.error("Destination world dose not exist!");
			return entity;
		}
		...
		if ((entity instanceof EntityPlayerMP) && interDimensional) {
			...
			if ((entity.addedToChunk) && (startWorld.getChunkFromBlockCoords(new BlockPos(entity.posX, entity.posY, entity.posZ)).isPopulated())) // todo
																																					// make
																																					// sure
																																					// this
																																					// isnt
		...
		if (interDimensional) {
			if (!(entity instanceof EntityPlayer)) {
				...
				if (entity == null) {
					LimeLib.log.error("Failed to teleport entity to new location");
					return null;
				}
